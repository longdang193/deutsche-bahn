...
from Environment_Knap_5 import KnapsackEnvironment
...
class MC_RL(Policy):
    ...
    def stochastic_action(self, state: dict, exploration_rate: bool) -> bool:
        ...
        """
        #TODO: write a stochastic action method to make decision either based on exploration or expolitation
        """
    ...
    def train(self,
        ...
        """
        #TODO: Write the train method
        1. Define an initial (training start) and final (training end) exploration rate
        2. Loop over training epochs
        3. Calculate an (exponential) exploration rate based on current epoch
        4. Set up one simulation run 
        5. Stick to the sequence reset, action, transition, action, transition etc. until action, terminated) and save the observed pds and rewards
        6. Attention: action selection is based on exploration rate: either random action (exploration) or actions based on lookup table (exploitation)
        --> define a method "stochastic_action" that, based on the current exploration rate, either return a random action or an action based on the lookup table
        6. When a simulation has terminated -> update the lookup table (last assignment)
        7. Plot training details if required
        8. Round the lookup table and save the lookup table and explored states table as a .txt file
        """
    ...
    def update_lookup(self, observed_pds: list, observed_rewards: list) -> None:
        ...
        for pds, fr_pds in zip(observed_pds, future_rewards_pds): # pds and related future rewards
            ...
            td: float = fr_pds - old_vf # calculate difference
            self.lookup_table[lookup_tuple] = old_vf + step_size * td # update value function in lookup table based on Monte Carlo update
    ...
    def evaluation(self, knapsack_env: KnapsackEnvironment, test_instances: int) -> float:
        ...
        """
        #TODO: write an evaluation method to test the intermediate policy performance
        """
