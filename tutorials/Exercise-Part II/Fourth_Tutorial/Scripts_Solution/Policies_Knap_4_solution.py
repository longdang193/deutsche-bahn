...
from Environment_Knap_4_solution import KnapsackEnvironment
from Item_Knap_4_solution import Item
import numpy as np
import time
import os
...
class MC_RL(Policy):
    ...
    def act(self, state: dict) -> bool:
        ...
        lookup_tuple_indices = (self.lookup_table.shape[0] - 1 - post_decision_take_cap, 
        cur_decision_point) # inverted index for capacity in lookup tables to have the lookup table created as in the slides
        post_decision_take_value = self.lookup_table[lookup_tuple_indices] # get the value of this pds via value function
        
        # post-decision state 2: reject item
        lookup_tuple_indices = (self.lookup_table.shape[0] - 1 - remaining_capacity, cur_decision_point) # inverted index in lookup tables (see above)
        post_decision_reject_value = self.lookup_table[lookup_tuple_indices] # get the value of this pds via value function
    ...
    def load_lookup_table(self) -> bool:
        ...
        if os.path.isfile("Fourth_Tutorial/Lookup_table.txt"): # check if pretrained lookup table is available
            print("Lookup table for Monte Carlo policy available")    
            self.lookup_table = np.loadtxt("Fourth_Tutorial/Lookup_table.txt") # load lookup table from working directory
            return True
        else:
            return False
