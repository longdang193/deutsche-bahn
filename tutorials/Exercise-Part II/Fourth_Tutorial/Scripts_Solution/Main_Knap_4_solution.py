...
from Policies_Knap_4_solution import Policy, Greedy, Threshold, Reserve_Cap, Sample, Perfect_Information, MC_RL
import pandas as pd
import time
...
if __name__ == "__main__":
    ...
    if "MC_RL" in evaluated_policies: # if monte carlo reinforcement learning policy should be evaluated
        ...
        initial_lookup_values: int = 100 # initial lookup table values
        ...
        if mc_rl.load_lookup_table(): # if lookup table is available
            ...
            raise FileNotFoundError("No lookup table for Monte Carlo policy available") # stop the programm
