...
from Environment_Knap_4 import KnapsackEnvironment
from Item_Knap_4 import Item
import numpy as np
import time
import os
...
class MC_RL(Policy):
    ...
    """
    #TODO: Write a monte carlo reinforcement learning class
    1. Define the constructor function -> think which parameters are required to initialize the lookup and observation table
    2. Initialize both required tables (lookup and observation) as introduced in the lecture
    """
         
    """
    #TODO: Write the act method
    1. Extract all information of the state
    2. Check feasibility
    3. Post Decision State 1: Take item -> get the corresponding PDS value from lookup table
    4. Post Decision State 2: Reject item -> get the corresponding PDS value from lookup table
    5. Check the properties of the bellman equation and return whether item is packed or not
    """
        
    """
    #TODO: Write a load method called "load_lookup_table" to load an available lookup table
    1. Check whether a lookup table exists in the working directory
    2. If it exists: load the lookup table and save it in the self variable for the lookup table initialized in the constructor
    3. Return True if table is available, False if not
    """
